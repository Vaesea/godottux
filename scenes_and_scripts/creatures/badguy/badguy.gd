...
func _ready() -> void:
	...
	if spiky and not sleeping:
		$WakeUpShapecast.enabled = false
		$WakeUpShapecast.visible = false
		$Image.play("walk")
	elif spiky and sleeping:
		$Image.play("sleep")
	elif not spiky and not sleeping:
		$Image.play("walk")
...
func _physics_process(delta: float) -> void:
	if walking_and_holdable:
		...
		# the thing after that "or" doesn't work???
		if current_iceblock_state == IceblockStates.MovingFlat or current_iceblock_state == IceblockStates.Held:
			kill_self_on_touching_enemy = true
			kill_other_enemies = true
	...
	if smart and not dead and is_on_floor():
...
func move():
	...
	if sleeping_spiky or dead:
		velocity.x = 0
		return
	elif spiky and not sleeping and not dead:
...
func _on_tux_detector_area_entered(area) -> void:
	if area.is_in_group("Stomp") and not spiky:
...
func _on_tux_detector_body_entered(body) -> void:
	if body.is_in_group("Player") and not dead:
		...
		body.queue_free()
		burn()
	elif body.is_in_group("Enemy") and not dead and kill_other_enemies:
		body.death(true)
